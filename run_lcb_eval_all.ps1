# Runs livecodebench_bench.py against all three models in turn, unattended.
# Same pattern as run_eval_all.ps1, different eval. See README "Benchmark
# methodology" for what this is and its contamination-reduction caveat.
#
# Usage: powershell -File run_lcb_eval_all.ps1
...
foreach ($m in $Models) {
    ...
    $logPath = Join-Path $ModelsDir "lcb_$($m.Label)_server.log"
    ...
    python livecodebench_bench.py $m.Label
